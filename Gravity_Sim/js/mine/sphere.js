...
function sphere(geo, matter)
{
	...
	this.size = [400];
	this.velVec = [ new THREE.Vector3(v, v, v) ];
	this.posGeo = [ new THREE.Mesh( geo, matter ) ];
	this.position = [ new THREE.Vector3(dx, dy, 0) ];
	
	this.posGeo[0].position.copy(this.position[0]);
	this.mass = [];//[ 4/3*Math.PI*Math.pow( this.size[0]/2, 3 ) *dense ];
	
	this.size[1] = 400;
	this.velVec[1] = new THREE.Vector3(-v, -v, -v);
	this.posGeo[1] = new THREE.Mesh( geo, matter );
	this.position[1] = new THREE.Vector3(-dx, -dy, 0);
	this.posGeo[1].position.copy(this.position[1]);

	/*this.size[2] = 400;
	this.velVec[2] = new THREE.Vector3(-v, v, v);
	this.posGeo[2] = new THREE.Mesh( geo, matter );
	this.position[2] = new THREE.Vector3(d, d, 0);
	this.posGeo[2].position.copy(this.position[2]);
	
	this.size[3] = 400;
	this.velVec[3] = new THREE.Vector3(-v, -v, -v);
	this.posGeo[3] = new THREE.Mesh( geo, matter );
	this.position[3] = new THREE.Vector3(0, d, 0);
	this.posGeo[3].position.copy(this.position[3]);*/
...
{
	var dense = Math.random()*50+2.5;
	this.size.push(Math.round(Math.random()*55+10));
	this.mass.push((4/3)*Math.PI*Math.pow(this.size[this.count]/2, 3)*dense);
	
	var geo = new THREE.SphereGeometry(this.size[this.count] - 5, 18, 18);
	...
	var x = (Math.random()-0.5)*2800, y = (Math.random()-0.5)*2800, z = (Math.random()-0.5)*2800;
...
sphere.prototype.ForceOfAcc = function(indexA, indexB)
{
	...
	var distmassA = ( -d*this.mass[indexA] );
	this.velVec[indexA].add(new THREE.Vector3(accvec.x/distmassA, accvec.y/distmassA, accvec.z/distmassA));
...
sphere.prototype.newVect = function(elem1, elem2, d){
    ...
    //the unit Norm
	var unitNorm = new THREE.Vector3();
	unitNorm = norm.divideScalar(d);
    ...
    this.velVec[elem1].add(new THREE.Vector3(impulse.x*ratio, impulse.y*ratio, impulse.z*ratio));
...
sphere.prototype.distanceTo = function(bees){

	return Math.sqrt(Math.pow(this.x-bees.x, 2) + Math.pow(this.y-bees.y, 2) + Math.pow(this.z - bees.z, 2));
}
